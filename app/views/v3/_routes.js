...
router.post('/application/duly-making', (req, res) => {
  ...
  applicationToEdit[0].dulyMade = 'true'
...
router.get('/application/summary', (req, res, next) => {
  ...
  delete req.session.data['unassign']
...
const startApplication = function (req, res) {
  // Grab the current application
  let applicationToEdit = req.application
...
const assignApplication = function (req, res) {
  ...
  if (req.session.data['unassign']) {
    delete applicationToEdit[0].owner
  } else {
    applicationToEdit[0].owner = owner
  }

  if (req.session.data['assign-start'] == 'true') {
  ...
  delete req.session.data['assign-start']
...
router.get('/application/assign-to-me', (req, res) => {
  if (req.application[0].status == 'Duly made') {
    startApplication(req, res)
  }
...
router.get('/application/unassign', (req, res) => {
  assignApplication(req, res)
...
router.post('/application/query', (req, res) => {
  ...
  // Assign the application to user if there is no owner
  if (!applicationToEdit[0].owner) {
    assignApplication(req, res)
    delete req.session.data['assigned']
  }
  ...
  applicationToEdit[0].returnStatus = applicationToEdit[0].status
...
router.get('/application/query-response', (req, res) => {
  ...
  const newStatus = Object.assign({
    ...
    date: new Date(),
    old: applicationToEdit[0].status,
    new: applicationToEdit[0].returnStatus
  })
  // Pass the new status into the audit log
  applicationToEdit[0].audit.unshift(newStatus)

  // Update the status of the application and change the SIP
  applicationToEdit[0].status = applicationToEdit[0].returnStatus
  applicationToEdit[0].sip = 'si-plan.pdf'

  delete applicationToEdit[0].returnStatus
...
router.post('/application/decision', (req, res) => {
  ...
  const log = Object.assign({
    ...
    date: new Date(),
    old: applicationToEdit[0].status,
    new: req.session.data['decision'] || 'Approved'
  })
  ...
  // Update the application
  applicationToEdit[0].status = req.session.data['decision'] || 'Approved'
  applicationToEdit[0].complete = true
  applicationToEdit[0].due = 'n/a'
  delete applicationToEdit[0].assignment
